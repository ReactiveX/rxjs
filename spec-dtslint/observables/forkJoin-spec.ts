...
it('should infer correctly for array of 2 observables', () => {
  ...
  const res = forkJoin(a); // $ExpectType Observable<(Observable<string> | Observable<number>)[]>
});

it('should infer correctly for array of 3 observables', () => {
  const a = [of(1, 2, 3), of('a', 'b', 'c'), of(true, true, false)];
  const res = forkJoin(a); // $ExpectType Observable<(Observable<string> | Observable<number> | Observable<boolean>)[]>
});
