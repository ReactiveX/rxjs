...
import { queueScheduler as rxQueueScheduler, of, scheduled } from 'rxjs';
...
describe('combineAll operator', () => {
  ...
  it('should combine the provided observables', () => {
    rxTestScheduler.run(({ hot, expectObservable }) => {
      ...
      const combined = scheduled([firstSource, secondSource], rxTestScheduler)
        .pipe(combineAll((a, b) => '' + a + b));
  ...
  it('should accept array of observables', () => {
    rxTestScheduler.run(({ hot, expectObservable }) => {
      ...
      const combined = scheduled([firstSource, secondSource], rxTestScheduler)
        .pipe(combineAll((a: string, b: string) => '' + a + b));
  ...
  it('should work with two nevers', () => {
    rxTestScheduler.run(({ cold, expectObservable, expectSubscriptions }) => {
      ...
      const result = scheduled([e1, e2], rxTestScheduler).pipe(combineAll((x, y) => x + y));
  ...
  it('should work with hot and throw', () => {
    rxTestScheduler.run(({ hot, expectObservable, expectSubscriptions }) => {
      ...
      const result = scheduled([e1, e2], rxTestScheduler).pipe(combineAll((x: number, y: string) => x + y));
  ...
  it('should work with throw and hot', () => {
    rxTestScheduler.run(({ hot, expectObservable, expectSubscriptions }) => {
      ...
      const result = scheduled([e1, e2], rxTestScheduler).pipe(combineAll((x: string, y: number) => x + y));
  ...
  it('should work with error and throw', () => {
    rxTestScheduler.run(({ hot, expectObservable, expectSubscriptions }) => {
      ...
      const result = scheduled([e1, e2], rxTestScheduler).pipe(combineAll((x: number|string, y: string) => x + y));
  ...
  it('should work with throw and error', () => {
    rxTestScheduler.run(({ hot, expectObservable, expectSubscriptions }) => {
      ...
      const result = scheduled([e1, e2], rxTestScheduler).pipe(combineAll((x: string, y: number|string) => x + y));
  ...
  it('should handle throw after complete left', () => {
    rxTestScheduler.run(({ hot, expectObservable, expectSubscriptions }) => {
      const e1 = hot(' --a--^--b---|', { a: 1, b: 2 });
      const leftSubs = '      ^------!';
      const e2 = hot('-----^--------#', undefined, 'bad things');
      const rightSubs = '     ^--------!';
      const expected = '      ---------#';
      ...
      expectSubscriptions(e1.subscriptions).toBe(leftSubs);
      expectSubscriptions(e2.subscriptions).toBe(rightSubs);
    });
  });

  it('should handle throw after complete right', () => {
    rxTestScheduler.run(({ hot, expectObservable, expectSubscriptions }) => {
      const e1 = hot(' -----^--------#', undefined, 'bad things');
      const leftSubs = '      ^--------!';
      const e2 = hot('--a--^--b---|', { a: 1, b: 2 });
      const rightSubs = '     ^------!';
      const expected = '      ---------#';
  ...
  it('should handle two consecutive hot observables', () => {
    rxTestScheduler.run(({ hot, expectObservable, expectSubscriptions }) => {
      ...
      const e2 = hot('-----^----------d---e--f-|', { d: 'd', e: 'e', f: 'f' });
      const e2subs = '     ^-------------------!';
      const expected = '   -----------(bc)y--z-|';

      const result = scheduled([e1, e2], rxTestScheduler).pipe(combineAll((x, y) => x + y));

      expectObservable(result).toBe(expected, { b: 'bd', c: 'cd', y: 'ce', z: 'cf' });
      expectSubscriptions(e1.subscriptions).toBe(e1subs);
      expectSubscriptions(e2.subscriptions).toBe(e2subs);
    });
  });
  ...
  it('should handle two consecutive hot observables with error left', () => {
    rxTestScheduler.run(({ hot, expectObservable, expectSubscriptions }) => {
      const e1 = hot(' --a--^--b--c--#', { a: 'a', b: 'b', c: 'c' }, 'jenga');
      const leftSubs = '      ^--------!';
      const e2 = hot('-----^----------d--e--f--|', { d: 'd', e: 'e', f: 'f' });
      const rightSubs = '     ^--------!';
      const expected = '      ---------#';
  ...
  it('should handle two consecutive hot observables with error right', () => {
    rxTestScheduler.run(({ hot, expectObservable, expectSubscriptions }) => {
      const e1 = hot(' --a--^--b--c--|', { a: 'a', b: 'b', c: 'c' });
      const leftSubs = '      ^--------!';
      const e2 = hot('-----^----------d---e--f-#', { d: 'd', e: 'e', f: 'f' }, 'dun dun dun');
      const rightSubs = '     ^-------------------!';
      const expected = '      -----------(ax)y--z-#';

      const result = scheduled([e1, e2], rxTestScheduler).pipe(combineAll((x, y) => x + y));

      expectObservable(result).toBe(expected, { a: 'bd', x: 'cd', y: 'ce', z: 'cf' }, 'dun dun dun');
      expectSubscriptions(e1.subscriptions).toBe(leftSubs);
      expectSubscriptions(e2.subscriptions).toBe(rightSubs);
    });
  });
  ...
  it('should handle selector throwing', () => {
    rxTestScheduler.run(({ hot, expectObservable, expectSubscriptions }) => {
      ...
      const result = scheduled([e1, e2], rxTestScheduler)
        .pipe(combineAll((x, y) => { throw 'ha ha ' + x + ', ' + y; }));
