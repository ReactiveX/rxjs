...
describe('ReplaySubject', () => {
  ...
  it('should replay values upon subscription', (done: Mocha.Done) => {
    const subject = new ReplaySubject<number>();
    const expects = [1, 2, 3];
    let i = 0;
    subject.next(1);
  ...
  it('should replay values and complete', (done: Mocha.Done) => {
    const subject = new ReplaySubject<number>();
    const expects = [1, 2, 3];
    let i = 0;
    subject.next(1);
  ...
  it('should replay values and error', (done: Mocha.Done) => {
    const subject = new ReplaySubject<number>();
    const expects = [1, 2, 3];
    let i = 0;
    subject.next(1);
  ...
  it('should only replay values within its buffer size', (done: Mocha.Done) => {
  ...
  describe('with bufferSize=2', () => {
    it('should replay 2 previous values when subscribed', () => {
      rxTestScheduler.run(({ hot, expectObservable }) => {
        ...
        expectObservable(hot(sourceTemplate).pipe(tap(feedNextIntoSubject, feedErrorIntoSubject, feedCompleteIntoSubject))).toBe(
          sourceTemplate
        );
