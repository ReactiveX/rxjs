...
describe('Subscription', () => {
  describe('add()', () => {
    ...
    it('should unsubscribe a finalization function that was passed', () => {
      let isCalled = false;
      const main = new Subscription();
      main.add(() => {
        isCalled = true;
    ...
    it('should unsubscribe a finalization function that was passed immediately if it has been unsubscribed', () => {
      let isCalled = false;
      const main = new Subscription();
      main.unsubscribe();
      main.add(() => {
  ...
  describe('remove()', () => {
    ...
    it('should remove added functions', () => {
      ...
      main.add(finalization);
      main.remove(finalization);
      main.unsubscribe();
      expect(isCalled).to.be.false;
    });
  ...
  describe('unsubscribe()', () => {
    it('should unsubscribe from all subscriptions, when some of them throw', (done) => {
      ...
      const source1 = new Observable(() => {
        return () => {
          finalizations.push(1);
        };
      });

      const source2 = new Observable(() => {
        return () => {
          finalizations.push(2);
      ...
      setTimeout(() => {
        ...
        expect(finalizations).to.deep.equal([1, 2, 3]);
        done();
      });
    });
    ...
    it('should unsubscribe from all parents', () => {
      ...
      b.add(d);
      c.add(d);
      // When d is added to the subscriptions, it's added as a finalization. The
      // length is 1 because the finalizations passed to the ctors are stored in a
      // separate property.
      expect((a as any)._finalizations).to.have.length(1);
      expect((b as any)._finalizations).to.have.length(1);
      expect((c as any)._finalizations).to.have.length(1);
      d.unsubscribe();
      // When d is unsubscribed, it should remove itself from each of its
      // parents.
      expect((a as any)._finalizations).to.have.length(0);
      expect((b as any)._finalizations).to.have.length(0);
      expect((c as any)._finalizations).to.have.length(0);
    });
  });
});
