 ...
 */
export function join<T1, T2, R>(right: Observable<T2>,
...
export interface JoinSignature<T> {
  <T2, R>(right: Observable<T2>, leftDurationSelector: (value: T) => Observable<any>,
          rightDurationSelector: (value: T2) => Observable<any>,
...
class JoinOperator<T1, T2, R> implements Operator<any, R> {
  ...
  call(subscriber: Subscriber<R>): Subscriber<any> {
    return new JoinSubscriber(subscriber, this.leftDurationSelector, this.rightDurationSelector, this.resultSelector);
  }
}
