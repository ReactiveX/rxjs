...
// let root = global || window || new Function('return this')(),

import { root } from './root';

export function defineImmediate(root) {
  let Immediate = {
    setImmediate: (x) => { return 0; },
    clearImmediate: (id) => {}
  };

  if (root && root.setImmediate) {
    Immediate.setImmediate = root.setImmediate;
    Immediate.clearImmediate = root.clearImmediate;
  } else {
...
export const Immediate = defineImmediate(root);