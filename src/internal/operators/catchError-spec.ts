...
import { createObservableInputs } from 'rxjs/internal/test_helpers/createObservableInputs';
...
describe('catchError operator', () => {
  ...
  it('should accept selector returns any ObservableInput', (done: MochaDone) => {
    ...
    input$.pipe(
      mergeMap(input =>
        throwError('bad').pipe(catchError(() => input))
      )
    ).subscribe(x => {
  ...
  context('fromPromise', () => {
    it('should chain a throw from a promise using throwError', (done: MochaDone) => {
      let nexted = false;
      let errored = false;
      let thrown: any;
      ...
      setTimeout(() => {
        ...
        expect(thrown).to.equal(thrownError);
        done();
      }, 0);
    });
  });
