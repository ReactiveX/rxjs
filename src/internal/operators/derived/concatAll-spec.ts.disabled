...
import { from, throwError, of, fromScheduled } from 'rxjs';
...
describe('concatAll operator', () => {
  ...
  it('should be able to work on a different scheduler', () => {
    testScheduler.run(({ hot, cold, expectObservable, expectSubscriptionsTo }) => {
      ...
      const result = fromScheduled([e1, e2, e3], testScheduler).pipe(concatAll());
  ...
  it('should concatAll a nested observable with a single inner observable, and a scheduler', () => {
    testScheduler.run(({ hot, cold, expectObservable, expectSubscriptionsTo }) => {
      ...
      const result = fromScheduled([e1], testScheduler).pipe(concatAll());
