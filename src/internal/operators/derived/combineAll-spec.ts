...
describe('combineAll operator', () => {
  ...
  it('should combine events from two observables', () => {
    testScheduler.run(({ hot, cold, expectObservable, expectSubscriptionsTo }) => {
      ...
      const result = outer.pipe(combineAll());

      expectObservable(result).toBe(expected, { A: ['a', '1'], B: ['a', '2'], C: ['b', '2'] });
    });
  });
  ...
  it('should work with two nevers', () => {
    testScheduler.run(({ hot, cold, expectObservable, expectSubscriptionsTo }) => {
      ...
      const result = of(e1, e2).pipe(combineAll());
  ...
  it('should work with hot-single and hot-empty', () => {
    testScheduler.run(({ hot, cold, expectObservable, expectSubscriptionsTo }) => {
      ...
      const result = of(e2, e1).pipe(combineAll());
  ...
  it('should combine 3 observables', () => {
    testScheduler.run(({ hot, cold, expectObservable, expectSubscriptionsTo }) => {
      ...
      const result = of(e1, e2, e3).pipe(combineAll());

      expectObservable(result).toBe(expected, { w: ['b', 'f', 'i'], x: ['c', 'f', 'i'], y: ['c', 'g', 'i'], z: ['c', 'g', 'j'] });
      expectSubscriptionsTo(e1).toBe(e1subs);
      expectSubscriptionsTo(e2).toBe(e2subs);
  ...
  it('should combine two immediately-scheduled observables', (done) => {
    ...
    fromScheduled([a, b], queueScheduler).pipe(combineAll())
      .subscribe((vals) => {
