...
The exact `sampleTime` Symbol likewise uses a host interval and emits only the
latest value received since the preceding tick. `timestamp` and `timeInterval`
read `Date.now()` by default and accept the narrow timestamp-provider contract
