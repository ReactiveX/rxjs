...
D-060 proposes both surfaces and the first pilot proves only root `rx`, `map`,
and `filter`. The existing `rxjs/map` and `rxjs/filter` deep imports still
export exact Symbols. Before expanding the catalog, decide:
