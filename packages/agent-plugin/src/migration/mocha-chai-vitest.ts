...
function convertChaiAssertion(
  ...
  const expectCall = factory.createCallExpression(factory.createIdentifier('expect'), undefined, [
    ts.visitNode(chain.actual, visit) as ts.Expression,
  ]);
...
function findUnsupportedChaiProperties(sourceFile: ts.SourceFile, expectLocal: string | undefined): readonly ts.PropertyAccessExpression[] {
  if (!expectLocal) return [];
