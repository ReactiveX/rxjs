  ...
  const transformed = ts.transform(sourceFile, [
    (context) => {
      ...
      return (root) => {
        ...
        const removableOperatorLocals = new Set([...convertedOperatorLocals].filter((local) => !preservedOperatorLocals.has(local)));
...
function findManualTestSchedulerSetup(sourceFile: ts.SourceFile, typeLocals: ReadonlySet<string>): ts.ExpressionStatement | undefined {
    ...
    if (unsafeOperatorLocals.has(local)) {
      diagnostics.push(
        diagnosticForNode(sourceFile, operator.expression, {
          ...
          nextAction: {
            code: 'review-source',
            message: 'Review the binding and migrate every remaining use before removing its legacy import.',
          },
    ...
    if (schedulerArgument) {
      diagnostics.push(
        diagnosticForNode(sourceFile, schedulerArgument, {
          ...
          classification: 'compatibility-only',
          nextAction: {
            code: 'remove-unsupported-overload',
            message: 'Characterize the timing behavior before removing or replacing the scheduler.',
          },
          capabilityId: mapping.id,
        })
      );
...
function validateArity(sourceFile: ts.SourceFile, call: ts.CallExpression, mapping: CapabilityMapping): MigrationDiagnostic | undefined {
  ...
  return diagnosticForNode(sourceFile, node, {
    ...
    nextAction: {
      code: 'remove-unsupported-overload',
      message: 'Preserve the unsupported overload and migrate it manually after characterization.',
    },
    capabilityId: mapping.id,
  });
}
  ...
  for (const observation of observations.slice(1)) {
    diagnostics.push(
      diagnosticForNode(sourceFile, observation, {
        ...
        classification: 'compatibility-only',
        nextAction: {
          code: 'add-characterization-test',
          message: 'Verify cold versus shared platform behavior before accepting the migration.',
        },
      })
    );
  }
