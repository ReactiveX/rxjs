...
export interface ExpectedDiagnostic {
  ...
  readonly classification: 'portable' | 'harness-rewrite' | 'compatibility-only' | 'intentional-divergence' | 'unsupported-or-obsolete';
...
export const mechanicalFixtures: readonly MechanicalFixture[] = [
  {
    ...
    expected: 'const result = source.filter(value => value > 1);\n',
  ...
  },
  {
    ...
    expected: 'const result = source.map(value => value + 1);\n',
    expectedStatus: 'changed',
    expectedDiagnostics: [],
    exactOutput: true,
  ...
  },
  {
    ...
    input: "import { map, shareReplay } from 'rxjs/operators';\nconst result = source.pipe(map(value => value), shareReplay(1));\n",
    expected: "import { map, shareReplay } from 'rxjs/operators';\nconst result = source.pipe(map(value => value), shareReplay(1));\n",
    expectedStatus: 'refused',
    expectedDiagnostics: [
      {
        code: 'unsafe-binding',
