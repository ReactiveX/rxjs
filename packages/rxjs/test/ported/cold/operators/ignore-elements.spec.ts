...
describe('ignoreElements (cold)', () => {
  it('should ignore all the elements of the source', async () => {
    await rxTest(({ hot, expectObservable, expectSubscriptions }) => {
      const e1 = hot('  --a--b--c--d--|');
      const e1subs = '  ^-------------!';
      const expected = '--------------|';
      expectObservable(e1[mergeMap](() => ColdObservable.from([]))).toBe(expected);
  ...
  it('should allow unsubscribing early and explicitly', async () => {
    await rxTest(({ hot, expectObservable, expectSubscriptions }) => {
      ...
      const expected = '--------       ';
      const unsub = '   -------!       ';
      const result = e1[mergeMap](() => ColdObservable.from([]));
  ...
  it('should handle never', async () => {
    await rxTest(({ cold, expectObservable, expectSubscriptions }) => {
      ...
      const expected = '-';
      expectObservable(
        e1[mergeMap](() => ColdObservable.from([])),
        '^!'
      ).toBe(expected);
      expectSubscriptions(e1.subscriptions).toBe(e1subs);
    });
