    ...
    await rxTest(async ({ cold, hot, expectObservable, expectSubscriptions, flush, now, schedule }) => {
        ...
        new ColdObservable((subscriber) => {
          schedule(
            () => {
              ...
              ColdObservable.from(input).subscribe(subscriber, {
      ...
      const result = source[expand](
        (value) => {
          ...
          return scheduleSubscription(value === 8 ? EMPTY : cold('--c|', { c: 2 })[map]((innerValue) => innerValue * value));
        },
        { concurrent: Infinity }
      );
  ...
  it('should map and recursively flatten with scalars', async () => {
    await rxTest(({ hot, expectObservable, expectSubscriptions }) => {
      ...
      const result = e1[expand]((x) => {
        ...
        return ColdObservable.from([x + x]); // scalar
      });
      expectObservable(result).toBe(expected, values);
      expectSubscriptions(e1.subscriptions).toBe(e1subs);
    });
