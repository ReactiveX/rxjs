...
// Source: spec/operators/combineLatestAll-spec.ts
import { describe, it } from 'vitest';
import { rxTest } from '@rxjs/test';
import { ColdObservable } from 'rxjs/cold-observable';
import { combineLatestAll } from 'rxjs/combine-latest-all';
import { mergeMap } from 'rxjs/merge-map';
describe('combineLatestAll (cold)', () => {
  it('should combine events from two observables', async () => {
    await rxTest(({ hot, cold, expectObservable }) => {
      const x = cold('                  -a-----b---|');
      const y = cold('                  --1-2-|     ');
      const outer = hot('-x----y--------|           ', { x: x, y: y });
      const expected = ' -----------------A-B--C---|';
      const result = outer[combineLatestAll]((a, b) => String(a) + String(b));
  ...
  it('should work with two nevers', async () => {
    await rxTest(({ cold, expectObservable, expectSubscriptions }) => {
      ...
      const e2subs = '^!';
      const expected = '-';
      const result = ColdObservable.from([e1, e2])[combineLatestAll]((x, y) => x + y);
  ...
  it('should work with hot-single and hot-empty', async () => {
    await rxTest(({ hot, expectObservable, expectSubscriptions }) => {
      ...
      const e2subs = '   ^---!';
      const expected = ' ----|';
      const result = ColdObservable.from([e2, e1])[combineLatestAll]((x, y) => x + y);
      expectObservable(result).toBe(expected);
      expectSubscriptions(e1.subscriptions).toBe(e1subs);
      expectSubscriptions(e2.subscriptions).toBe(e2subs);
    });
  ...
  it('should combine 3 observables', async () => {
    await rxTest(({ hot, expectObservable, expectSubscriptions }) => {
      const e1 = hot('--a--^--b--c--|');
      const e1subs = '     ^--------!';
      const e2 = hot('---e-^---f--g--|');
      ...
      const e3subs = '     ^---------!';
      const expected = '   -----wxyz-|';
      const result = ColdObservable.from([e1, e2, e3])[combineLatestAll]((x, y, z) => x + y + z);
  ...
  it('should handle throw after complete left', async () => {
    await rxTest(({ hot, expectObservable, expectSubscriptions }) => {
      ...
      const rightSubs = '     ^--------!';
      const expected = '      ---------#';
      const result = ColdObservable.from([left, right])[combineLatestAll]((x, y) => x + y);
  ...
  it('should handle selector throwing', async () => {
    await rxTest(({ hot, expectObservable, expectSubscriptions }) => {
      ...
      const e2subs = '     ^--!';
      const expected = '   ---#';
      const result = ColdObservable.from([e1, e2])[combineLatestAll]((x, y) => {
        throw 'ha ha ' + x + ', ' + y;
      });
