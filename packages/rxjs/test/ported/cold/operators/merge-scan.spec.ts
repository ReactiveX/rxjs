...
describe('mergeScan (cold)', () => {
  it('should mergeScan things', async () => {
    await rxTest(({ hot, expectObservable, expectSubscriptions }) => {
      ...
      const result = e1[mergeScan]((acc, x) => ColdObservable.from([acc.concat(x)]), []);
  ...
  it('should mergeScan values and be able to asynchronously project them', async () => {
    await rxTest(({ hot, time, expectObservable, expectSubscriptions }) => {
      ...
      const result = e1[mergeScan]((acc, x) => ColdObservable.from([acc.concat(x)])[delay](t), []);
      expectObservable(result).toBe(expected, values);
      expectSubscriptions(e1.subscriptions).toBe(e1subs);
    });
  });
  ...
  it('should not break unsubscription chains when result is unsubscribed explicitly', async () => {
    await rxTest(({ hot, time, expectObservable, expectSubscriptions }) => {
      ...
      const result = e1[mergeMap]((x) => ColdObservable.from([x]))
        [mergeScan]((acc, x) => ColdObservable.from([acc.concat(x)])[delay](t), [])
  ...
  it('should handle errors in the projection function', async () => {
    await rxTest(({ hot, expectObservable, expectSubscriptions }) => {
      ...
      const result = e1[mergeScan]((acc, x) => {
        ...
        return ColdObservable.from([acc.concat(x)]);
      }, []);
