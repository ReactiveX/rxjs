...
describe('zipAll (cold)', () => {
  ...
  it('should combine two observables', async () => {
    await rxTest(({ hot, expectObservable, expectSubscriptions }) => {
      ...
      expectObservable(ColdObservable.from([a, b])[zipAll](), '^-----------------!').toBe(expected, values);
  ...
  it('should end once one observable completes and its buffer is empty', async () => {
    await rxTest(({ hot, expectObservable, expectSubscriptions }) => {
      ...
      expectObservable(ColdObservable.from([e1, e2, e3])[zipAll]()).toBe(expected, values);
  ...
  it('should zip them with values', async () => {
    await rxTest(({ hot, expectObservable, expectSubscriptions }) => {
      ...
      expectObservable(ColdObservable.from([e1, myIterator])[zipAll]()).toBe(expected, values);
      expectSubscriptions(e1.subscriptions).toBe(e1subs);
    });
  });
  it('should complete instantly with never observable and empty iterable', async () => {
    await rxTest(({ cold, expectObservable, expectSubscriptions }) => {
      ...
      const b = [];
      const expected = '|';
      expectObservable(ColdObservable.from([a, b])[zipAll]()).toBe(expected);
      expectSubscriptions(a.subscriptions).toBe(asubs);
    });
  });
  ...
  it('should work with non-empty observable and non-empty iterable', async () => {
    await rxTest(({ hot, expectObservable, expectSubscriptions }) => {
      ...
      const b = [2];
      const expected = '-----(x|)';
      expectObservable(ColdObservable.from([a, b])[zipAll]()).toBe(expected, { x: ['1', 2] });
      expectSubscriptions(a.subscriptions).toBe(asubs);
    });
  });
  ...
  it('should work with non-empty many observable and non-empty many iterable', async () => {
    await rxTest(({ hot, expectObservable, expectSubscriptions }) => {
      ...
      const b = [4, 5, 6];
      const expected = '---x--y--(z|)';
      expectObservable(ColdObservable.from([a, b])[zipAll]()).toBe(expected, { x: ['1', 4], y: ['2', 5], z: ['3', 6] });
      expectSubscriptions(a.subscriptions).toBe(asubs);
    });
  });
  it('should work with non-empty observable and non-empty iterable selector that throws', async () => {
    await rxTest(({ hot, expectObservable, expectSubscriptions }) => {
      ...
      expectObservable(ColdObservable.from([a, b])[zipAll](selector)).toBe(expected, { x: '14' }, new Error('too bad'));
      expectSubscriptions(a.subscriptions).toBe(asubs);
    });
  });
  it('should combine two observables and selector', async () => {
    await rxTest(({ hot, expectObservable, expectSubscriptions }) => {
      ...
      const expected = '---x---y---z';
      expectObservable(
        ColdObservable.from([a, b])[zipAll]((e1, e2) => e1 + e2),
        '^-----------------!'
  ...
  it('should work with n-ary symmetric', async () => {
    await rxTest(({ hot, expectObservable, expectSubscriptions }) => {
      ...
      const c = hot('   ---1-^---3---6-|  ');
      const expected = '     ----x---y-|  ';
      expectObservable(ColdObservable.from([a, b, c])[zipAll]()).toBe(expected, { x: ['1', '2', '3'], y: ['4', '5', '6'] });
      expectSubscriptions(a.subscriptions).toBe(asubs);
      expectSubscriptions(b.subscriptions).toBe(bsubs);
    });
  });
  it('should work with n-ary symmetric selector', async () => {
    await rxTest(({ hot, expectObservable, expectSubscriptions }) => {
      ...
      const c = hot('---1-^---3---6-|  ');
      const expected = '  ----x---y-|  ';
      const observable = ColdObservable.from([a, b, c])[zipAll]((r0, r1, r2) => [r0, r1, r2]);
  ...
  it('should work with some data asymmetric 1', async () => {
    await rxTest(({ hot, expectObservable, expectSubscriptions }) => {
      ...
      expectObservable(ColdObservable.from([a, b])[zipAll]((r1, r2) => r1 + r2)).toBe(expected, {
        a: '12',
        b: '34',
        c: '56',
        d: '78',
  ...
  it('should work with selector throws', async () => {
    await rxTest(({ hot, expectObservable, expectSubscriptions }) => {
      ...
      const observable = ColdObservable.from([a, b])[zipAll](selector);
  ...
  it('should work with error and error', async () => {
    await rxTest(({ hot, expectObservable, expectSubscriptions }) => {
      ...
      const bsubs = '   ^-----!';
      const expected = '------#';
      expectObservable(ColdObservable.from([a, b])[zipAll]()).toBe(expected, null, 'too bad');
      expectSubscriptions(a.subscriptions).toBe(asubs);
      expectSubscriptions(b.subscriptions).toBe(bsubs);
    });
  });
  it('should work with two sources that eventually raise errors', async () => {
    await rxTest(({ hot, expectObservable, expectSubscriptions }) => {
      ...
      const bsubs = '   ^-------!';
      const expected = '-----x--#';
      expectObservable(ColdObservable.from([a, b])[zipAll]()).toBe(expected, { x: [1, 2] }, 'too bad');
      expectSubscriptions(a.subscriptions).toBe(asubs);
      expectSubscriptions(b.subscriptions).toBe(bsubs);
    });
  });
  it('should work with two sources that eventually raise errors (swapped)', async () => {
    await rxTest(({ hot, expectObservable, expectSubscriptions }) => {
      ...
      const bsubs = '   ^-------!';
      const expected = '-----x--#';
      expectObservable(ColdObservable.from([a, b])[zipAll]()).toBe(expected, { x: [2, 1] }, 'too bad');
      expectSubscriptions(a.subscriptions).toBe(asubs);
      expectSubscriptions(b.subscriptions).toBe(bsubs);
    });
  });
  ...
  it('should not break unsubscription chain when unsubscribed explicitly', async () => {
    await rxTest(({ hot, expectObservable, expectSubscriptions }) => {
      ...
      const r = ColdObservable.from([a, b])
