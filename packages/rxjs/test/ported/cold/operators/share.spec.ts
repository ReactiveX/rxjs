...
describe('share (cold)', () => {
  ...
  it('should reset on refCount 0 when synchronously resubscribing and using a sync reset notifier', async () => {
    const syncNotify = ColdObservable.from([1]);
    await rxTest(({ hot, expectObservable, expectSubscriptions }) => {
      ...
      const result = ColdObservable[concat]([sharedSource, sharedSource]);
  ...
  it('should not reset on refCount 0 when synchronously resubscribing and using a deferred reset notifier', async () => {
    await rxTest(({ cold, expectObservable, expectSubscriptions, schedule }) => {
      const deferredNotify = new ColdObservable((subscriber) => {
        schedule(
          () => {
            subscriber.next(1);
            subscriber.complete();
  ...
  it('should reset on refCount 0 only after reset notifier emitted', async () => {
    await rxTest(({ hot, cold, expectObservable, expectSubscriptions }) => {
      ...
      const result = ColdObservable[concat]([sharedSource, firstPause, sharedSource, secondPause, sharedSource]);
      expectObservable(result, subscription).toBe(expected);
      expectSubscriptions(source.subscriptions).toBe(sourceSubs);
    });
  });
  ...
  it('should not reset on refCount 0 if reset notifier errors before emitting any value', async () => {
    const onUnhandledError = vi.fn();
    {
      ...
      await rxTest(({ hot, cold, expectObservable, expectSubscriptions }) => {
        ...
        const sharedSource = source[share]({ resetOnRefCountZero: () => reset })[take](2);
        const result = ColdObservable[concat]([sharedSource, firstPause, sharedSource]);
  ...
  it('should not call "resetOnRefCountZero" on error', async () => {
    await rxTest(({ cold, expectObservable, expectSubscriptions }) => {
      ...
      const result = ColdObservable[concat]([sharedSource[onErrorResumeNext]([firstPause]), sharedSource]);
      expectObservable(result, subscription).toBe(expected);
      expectSubscriptions(source.subscriptions).toBe(sourceSubs);
  ...
  it('should mirror a simple source Observable [share() using sync reset notifiers equivalent to default config and notifying again after reset is notified]', async () => {
    const syncNotify = ColdObservable.from([1]);
    const options = {
      resetOnError: () => ColdObservable[concat]([syncNotify, syncNotify]),
      resetOnComplete: () => ColdObservable[concat]([syncNotify, syncNotify]),
      resetOnRefCountZero: () => ColdObservable[concat]([syncNotify, syncNotify]),
    };
    await rxTest(({ cold, expectObservable, expectSubscriptions }) => {
      const source = cold('--1-2---3-4--5-|');
      const sourceSubs = ' ^--------------!';
  ...
  it('should mirror a simple source Observable [share() using sync reset notifiers equivalent to default config and never completing after reset is notified]', async () => {
    const syncNotify = ColdObservable.from([1]);
    const options = {
      resetOnError: () => ColdObservable[concat]([syncNotify, NEVER]),
      resetOnComplete: () => ColdObservable[concat]([syncNotify, NEVER]),
      resetOnRefCountZero: () => ColdObservable[concat]([syncNotify, NEVER]),
    };
    await rxTest(({ cold, expectObservable, expectSubscriptions }) => {
      const source = cold('--1-2---3-4--5-|');
      const sourceSubs = ' ^--------------!';
  ...
  it('should mirror a simple source Observable [share() using sync reset notifiers equivalent to default config and throwing an error after reset is notified]', async () => {
    const syncNotify = ColdObservable.from([1]);
    const syncError = new ColdObservable((subscriber) => {
      subscriber.error(new Error());
    });
    const options = {
      resetOnError: () => ColdObservable[concat]([syncNotify, syncError]),
      resetOnComplete: () => ColdObservable[concat]([syncNotify, syncError]),
      resetOnRefCountZero: () => ColdObservable[concat]([syncNotify, syncError]),
    };
    await rxTest(({ cold, expectObservable, expectSubscriptions }) => {
      const source = cold('--1-2---3-4--5-|');
      const sourceSubs = ' ^--------------!';
  ...
  it('should not break unsubscription chain when last subscriber unsubscribes [share()]', async () => {
    ...
    await rxTest(({ cold, hot, expectObservable, expectSubscriptions }) => {
      ...
      const unsub2 = '         ------------!   ';
      const expected2 = '      --------3----   ';
      const shared = source[mergeMap]((x) => ColdObservable.from([x]))
        [share](options)
  ...
  it('should not fail on reentrant subscription [share()]', async () => {
    ...
    await rxTest(({ cold, expectObservable, expectSubscriptions }) => {
      ...
      const subs = '       (^!)  ';
      const expected = '   (136|)';
      const deferred = new ColdObservable((subscriber) => {
        ColdObservable.from(shared).subscribe(subscriber, { signal: subscriber.signal });
      })[startWith](0);
