...
describe('forkJoin (cold)', () => {
  it('should join the last values of the provided observables into an array', async () => {
    await rxTest(({ hot, cold, expectObservable }) => {
      ...
      const e1 = ColdObservable[forkJoin]([s1, s2, s3]);
      const expected = '--------------------(x|)';
  ...
  it('should accept single observable', async () => {
    await rxTest(({ hot, expectObservable }) => {
      const e1 = ColdObservable[forkJoin](hot('--a--b--c--d--|'));
      const expected = '       --------------(x|)';
  ...
  it('should allow emit null or undefined', async () => {
    await rxTest(({ hot, expectObservable }) => {
      const e2 = ColdObservable[forkJoin]([
        hot('            --a--b--c--d--|', { d: null }),
        hot('            (b|)'),
        hot('            --1--2--3--|'),
  ...
  it('should accept array of observable contains single', async () => {
    await rxTest(({ hot, expectObservable }) => {
      const s1 = hot('  --a--b--c--d--|');
      const e1 = ColdObservable[forkJoin]([s1]);
      const expected = '--------------(x|)';
      expectObservable(e1).toBe(expected, { x: ['d'] });
    });
  });
  ...
  it('should complete when all sources are empty', async () => {
    await rxTest(({ hot, expectObservable }) => {
      const s1 = hot('  --------------|');
      const s2 = hot('  ---------|');
      const e1 = ColdObservable[forkJoin]([s1, s2]);
      const expected = '---------|';
      expectObservable(e1).toBe(expected);
    });
  });
  it('should not complete when only source never completes', async () => {
    await rxTest(({ hot, expectObservable }) => {
      const e1 = ColdObservable[forkJoin]([hot('--------------')]);
      const expected = '        --------------';
      expectObservable(e1, '^-------------!').toBe(expected);
    });
  });
  ...
  it('should complete if source is not provided', async () => {
    await rxTest(({ expectObservable }) => {
      const e1 = ColdObservable[forkJoin]();
      const expected = '|';
      expectObservable(e1).toBe(expected);
    });
  });
  ...
  it('should allow unsubscribing early and explicitly', async () => {
    await rxTest(({ hot, expectObservable, expectSubscriptions }) => {
      ...
      const expected = '   ----------    ';
      const unsub = '      ---------!    ';
      const result = ColdObservable[forkJoin]([e1, e2]);
  ...
  it('should allow unsubscribing early and explicitly', async () => {
    await rxTest(({ hot, expectObservable, expectSubscriptions }) => {
      ...
      const expected = '   ----------    ';
      const unsub = '      ---------!    ';
      const result = ColdObservable[forkJoin]({
        e1,
        e2,
      });
      expectObservable(result, unsub).toBe(expected);
