...
describe('race (cold)', () => {
  it('should race a single observable', async () => {
    await rxTest(({ cold, expectObservable, expectSubscriptions }) => {
      ...
      const result = ColdObservable[race]([e1]);
  ...
  it('should race cold and cold', async () => {
    await rxTest(({ cold, expectObservable, expectSubscriptions }) => {
      ...
      const e2subs = '  ^--!                    ';
      const expected = '---a-----b-----c----|   ';
      const result = ColdObservable[race]([e1, e2]);
      expectObservable(result).toBe(expected);
      expectSubscriptions(e1.subscriptions).toBe(e1subs);
  ...
  it('should not break unsubscription chains when unsubscribed explicitly', async () => {
    await rxTest(({ hot, expectObservable, expectSubscriptions }) => {
      ...
      const expected = '     ---b--c---    ';
      const unsub = '        ---------!    ';
      const result = ColdObservable[race]([e1[mergeMap]((x) => ColdObservable.from([x])), e2[mergeMap]((x) => ColdObservable.from([x]))])[
        mergeMap
      ]((x) => ColdObservable.from([x]));
      expectObservable(result, unsub).toBe(expected);
      expectSubscriptions(e1.subscriptions).toBe(e1subs);
      expectSubscriptions(e2.subscriptions).toBe(e2subs);
    });
  ...
  it('should never emit when given non emitting sources', async () => {
    await rxTest(({ cold, expectObservable, expectSubscriptions }) => {
      ...
      const e1subs = '  ^--!';
      const expected = '---|';
      const source = ColdObservable[race]([e1, e2]);
  ...
  it('handle empty', async () => {
    await rxTest(({ cold, expectObservable, expectSubscriptions }) => {
      ...
      const e1subs = '  (^!)';
      const expected = '|   ';
      const source = ColdObservable[race]([e1]);
      expectObservable(source).toBe(expected);
      expectSubscriptions(e1.subscriptions).toBe(e1subs);
    });
  });
