...
describe('combineLatest (cold)', () => {
  it('should combineLatest the provided observables', async () => {
    await rxTest(({ hot, expectObservable }) => {
      ...
      const combined = ColdObservable[combineLatest]([firstSource, secondSource], (a, b) => '' + a + b);
  ...
  it('should accept a dictionary of observables', async () => {
    await rxTest(({ hot, expectObservable }) => {
      ...
      const secondSource = hot('--d--e--f--g--|');
      const expected = '        ----uv--wx-y--z----|';
      const combined = ColdObservable[combineLatest]({ a: firstSource, b: secondSource })[map](({ a, b }) => '' + a + b);
      expectObservable(combined).toBe(expected, { u: 'ad', v: 'ae', w: 'af', x: 'bf', y: 'bg', z: 'cg' });
    });
  });
  it('should work with two nevers', async () => {
    await rxTest(({ cold, expectObservable, expectSubscriptions }) => {
      ...
      const e2subs = '^!';
      const expected = '-';
      const result = ColdObservable[combineLatest]([e1, e2], (x, y) => x + y);
  ...
  it('should work with hot-single and hot-empty', async () => {
    await rxTest(({ hot, expectObservable, expectSubscriptions }) => {
      ...
      const e2subs = '   ^---!';
      const expected = ' ----|';
      const result = ColdObservable[combineLatest]([e2, e1], (x, y) => x + y);
      expectObservable(result).toBe(expected, values);
      expectSubscriptions(e1.subscriptions).toBe(e1subs);
      expectSubscriptions(e2.subscriptions).toBe(e2subs);
    });
  ...
  it('should handle throw after complete left', async () => {
    await rxTest(({ hot, expectObservable, expectSubscriptions }) => {
      ...
      const rightSubs = '     ^--------!';
      const expected = '      ---------#';
      const result = ColdObservable[combineLatest]([left, right], (x, y) => x + y);
  ...
  it('should handle selector throwing', async () => {
    await rxTest(({ hot, expectObservable, expectSubscriptions }) => {
      ...
      const e2subs = '     ^--!';
      const expected = '   ---#';
      const result = ColdObservable[combineLatest]([e1, e2], (x, y) => {
        throw 'ha ha ' + x + ', ' + y;
      });
  ...
  it('should not break unsubscription chains when unsubscribed explicitly', async () => {
    await rxTest(({ hot, expectObservable, expectSubscriptions }) => {
      ...
      const unsub = '      ---------!    ';
      const values = { x: 'bf', y: 'cf', z: 'cg' };
      const result = ColdObservable[combineLatest](
        [e1[mergeMap]((x) => ColdObservable.from([x])), e2[mergeMap]((x) => ColdObservable.from([x]))],
        (x, y) => x + y
