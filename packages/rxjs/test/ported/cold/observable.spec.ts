...
describe('Observable (cold)', () => {
  ...
  it('should compose through combineLatest', async () => {
    class MyCustomObservable extends ColdObservable {
      static from(source) {
    ...
    await rxTest(({ cold, expectObservable }) => {
      const first = cold('-a--b-----c-d-e-|');
      const other = cold('--1--2-3-4---|   ');
      ...
      expect(result instanceof MyCustomObservable).toBe('cold' !== 'cold');
      expect(result instanceof ColdObservable).toBe(true);
