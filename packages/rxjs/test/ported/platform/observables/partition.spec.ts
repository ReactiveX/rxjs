...
describe('partition (platform)', () => {
  it('should partition an observable of integers into even and odd', async () => {
    await rxTest(({ hot, expectSubscriptions, expectObservable }) => {
      ...
      const result = Observable[partition](e1, (x) => x % 2 === 1);
      ((result_1, expected_1) => {
  ...
  it('should partition an observable into two using a predicate', async () => {
    await rxTest(({ hot, expectSubscriptions, expectObservable }) => {
      ...
      })(Observable[partition](e1, predicate), expected);
      expectSubscriptions(e1.subscriptions).toBe([e1subs, e1subs]);
    });
  });
  ...
  it('should partition an observable into two using a bound predicate', async () => {
    await rxTest(({ hot, expectSubscriptions, expectObservable }) => {
      ...
      })(Observable[partition](e1, predicate.bind({ value: 'a' })), expected);
      expectSubscriptions(e1.subscriptions).toBe([e1subs, e1subs]);
    });
  });
  ...
  it('should partition into two observable with early unsubscription', async () => {
    await rxTest(({ hot, expectObservable, expectSubscriptions }) => {
      ...
      const result = Observable[partition](e1, predicate);
  ...
  it('should not break unsubscription chains when result is unsubscribed explicitly', async () => {
    await rxTest(({ hot, expectObservable, expectSubscriptions }) => {
      ...
      const unsub = '    -------!          ';
      const e1Pipe = e1[mergeMap]((x) => Observable.from([x]));
      const result = Observable[partition](e1Pipe, (x) => x === 'a');
