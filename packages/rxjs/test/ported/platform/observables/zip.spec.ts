...
// Source: spec/observables/zip-spec.ts
...
describe('zip (platform)', () => {
  ...
  it('should complete instantly if given an empty iterable', async () => {
    await rxTest(({ hot, expectObservable, expectSubscriptions }) => {
      const a = hot('---^----a--|');
      const asubs = '   (^!)     ';
      const b = [];
  ...
  it('should work with n-ary symmetric selector', async () => {
    await rxTest(({ hot, expectObservable, expectSubscriptions }) => {
      ...
      const c = hot('---1-^---3---6-|  ');
      const expected = '  ----x---y-|  ';
      const observable = zip([a, b, c], (r0, r1, r2) => [r0, r1, r2]);
  ...
  it('should work with some data asymmetric 1', async () => {
    await rxTest(({ hot, expectObservable, expectSubscriptions }) => {
      ...
      expectObservable(zip([a, b], (r1, r2) => r1 + r2)).toBe(expected, { a: '12', b: '34', c: '56', d: '78', e: '90' });
      expectSubscriptions(a.subscriptions).toBe(asubs);
      expectSubscriptions(b.subscriptions).toBe(bsubs);
    });
  });
  it('should work with some data asymmetric 2', async () => {
    await rxTest(({ hot, expectObservable, expectSubscriptions }) => {
      ...
      const bsubs = '     ^-----------------!    ';
      const expected = '  ---a--b--c--d--e--|    ';
      expectObservable(zip([a, b], (r1, r2) => r1 + r2)).toBe(expected, { a: '21', b: '43', c: '65', d: '87', e: '09' });
      expectSubscriptions(a.subscriptions).toBe(asubs);
      expectSubscriptions(b.subscriptions).toBe(bsubs);
    });
  });
