...
describe('animationFrames (platform)', () => {
  it('should animate', async () => {
    await rxTest(({ animate, observable, expectObservable, time }) => {
      animate('            ---x---x---x');
      const mapped = observable('-m          ');
      const tm = time('    -|          ');
      const ta = time('    ---|        ');
      const tb = time('    -------|    ');
      const tc = time('    -----------|');
      ...
      const subs = '^-----------!';
      const result = mapped[mergeMap](() => Observable[animationFrames]());
  ...
  it('should use any passed timestampProvider', async () => {
    ...
    await rxTest(({ animate, observable, expectObservable }) => {
      animate('            ---x---x---x');
      const mapped = observable('-m          ');
      const expected = '   ---a---b---c';
      const subs = '^-----------!';
      const result = mapped[mergeMap](() => Observable[animationFrames](timestampProvider));
      expectObservable(result, subs).toBe(expected, {
  ...
  it('should compose with take', async () => {
    await rxTest(({ animate, observable, expectObservable, time }) => {
      ...
      const ta = time('---|');
      const tb = time('-------|');
      const frames = Observable[animationFrames]()[take](2);
  ...
  it('should compose with takeUntil', async () => {
    await rxTest(({ animate, observable, expectObservable, hot, time }) => {
      animate('---x---x---x');
      const mapped = observable('-m');
      const tm = time('-|');
      const ta = time('---|');
      const tb = time('-------|');
      const signal = hot('^--------s--');
      const frames = Observable[animationFrames]()[takeUntil](signal);
      const result = mapped[mergeMap](() => frames);
      expectObservable(result, '^-----------!').toBe('---a---b', {
        a: { elapsed: ta - tm, timestamp: ta },
        b: { elapsed: tb - tm, timestamp: tb },
