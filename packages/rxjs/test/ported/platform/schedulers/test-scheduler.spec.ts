...
describe('TestScheduler (platform)', () => {
  ...
  it('should parse a marble string into a series of notifications and types', async () => {
    await rxTest(async ({ observable, flush, now }) => {
      const actual = [];
      const controller = new AbortController();
      observable('-------a---b---|', { a: 'A', b: 'B' }).subscribe(
        {
  ...
  it('should parse a marble string with an error', async () => {
    await rxTest(async ({ observable, flush, now }) => {
      const actual = [];
      const controller = new AbortController();
      observable('-------a---b---#', { a: 'A', b: 'B' }, 'omg error!').subscribe(
        {
          next: (value) => actual.push([now(), 'N', value]),
          error: (error) => actual.push([now(), 'E', error]),
          complete: () => actual.push([now(), 'C']),
  ...
  it('should default in the letter for the value if no value hash was passed', async () => {
    await rxTest(async ({ observable, flush, now, schedule }) => {
      const actual = [];
      const controller = new AbortController();
      observable('--a--b--c--').subscribe(
        {
          next: (value) => actual.push([now(), 'N', value]),
          error: (error) => actual.push([now(), 'E', error]),
          complete: () => actual.push([now(), 'C']),
  ...
  it('should ignore whitespace when runMode=true', async () => {
    await rxTest(async ({ observable, flush, now }) => {
      const actual = [];
      const controller = new AbortController();
      observable('  -a - b -    c |       ', { a: 'A', b: 'B', c: 'C' }).subscribe(
        {
          next: (value) => actual.push([now(), 'N', value]),
          error: (error) => actual.push([now(), 'E', error]),
          complete: () => actual.push([now(), 'C']),
  ...
  it('should support time progression syntax when runMode=true', async () => {
    await rxTest(async ({ observable, flush, now }) => {
      const actual = [];
      const controller = new AbortController();
      observable('10.2ms a 1.2s b 1m c|', { a: 'A', b: 'B', c: 'C' }).subscribe(
        {
          next: (value) => actual.push([now(), 'N', value]),
          error: (error) => actual.push([now(), 'E', error]),
          complete: () => actual.push([now(), 'C']),
  ...
  it('should parse a subscription marble string with an unsubscription', async () => {
    ...
    try {
      await rxTest(({ observable, expectObservable }) => {
        expectObservable(observable('-a'), '---^').toBe('----a');
      });
    } catch (error) {
      rejection = error;
    }
  ...
  it('should create a cold observable', async () => {
    await rxTest(async ({ observable: createCold, flush: flushMarbles }) => {
      const expected = ['A', 'B'];
  ...
  it('should create a cold observable', async () => {
    await rxTest(({ observable: createCold, expectObservable: expectMarbles }) => {
      const expected = [1, 2];
  ...
  it('should return an object with a toBe function', async () => {
    await rxTest(({ expectObservable: expectMarbles }) => {
      expect(
        ...
        })(expectMarbles(Observable.from([1]))).toBe
      ).toBeTypeOf('function');
    });
  });
  it('should append to flushTests array', async () => {
    await rxTest(async ({ observable, expectObservable, flush }) => {
      const expectation = expectObservable(observable('|'));
  ...
  it('should append to flushTests array', async () => {
    await rxTest(async ({ observable, expectObservable, expectSubscriptions, flush }) => {
      const source = observable('|');
      expectObservable(source).toBe('|');
  ...
  it('should assert subscriptions of a cold observable', async () => {
    await rxTest(({ observable: createCold, expectSubscriptions: expectSubscriptionMarbles }) => {
  ...
  it('should be awesome', async () => {
    await rxTest(({ observable: createCold, expectObservable: expectMarbles, expectSubscriptions: expectSubscriptionMarbles }) => {
  ...
  it('should support testing metastreams', async () => {
    await rxTest(({ observable: createCold, hot: createHot, expectObservable: expectMarbles }) => {
  ...
  it('should ignore whitespace', async () => {
    await rxTest(({ observable, expectObservable, expectSubscriptions }) => {
      const input = observable('  -a - b -    c |       ');
      const output = input[mergeMap]((value) => Observable.from([value])[delay](10), { concurrent: 1 });
  ...
  it('should support time progression syntax', async () => {
    ...
    await rxTest(({ observable, expectObservable }) => {
      const output = observable('10.2ms a 1.2s b 1m c|');
      const expected = '   10.2ms a 1.2s b 1m c|';
      expectObservable(output).toBe(expected);
    });
  });
  it('should provide the correct helpers', async () => {
    await rxTest(({ observable, hot, flush, expectObservable, expectSubscriptions }) => {
  ...
  it('should have each frame represent a single virtual millisecond', async () => {
    ...
    await rxTest(({ observable, expectObservable }) => {
      const output = observable('-a-b-c--------|')[debounce](5);
  ...
  it('should have no maximum frame count', async () => {
    await rxTest(({ observable, expectObservable }) => {
      ...
      expectObservable(observable('-a|')[delay](10000)).toBe('- 10s (a|)');
    });
  });
  ...
  it('should flush automatically', async () => {
    await rxTest(({ observable, expectObservable }) => {
      const output = observable('-a-b-c|')[mergeMap]((value) => Observable.from([value])[delay](10), { concurrent: 1 });
  ...
  it('should flush expectations correctly', async () => {
    ...
    await (async () => {
      ...
      try {
        await (async () => {
          await rxTest(async ({ observable, expectObservable, flush }) => {
            expectObservable(observable('-x')).toBe('-x');
            expectObservable(observable('-y')).toBe('-y');
            const expectation = expectObservable(observable('-z'));
            await flush();
            expectation.toBe('-q');
          });
        })();
  ...
  it('should support animationFrame, async and asap schedulers', async () => {
    await rxTest(async ({ animate, observable, flush, now }) => {
      animate([9]);
      const values = [];
