...
describe('bufferToggle (platform)', () => {
  ...
  it('should emit buffers that are opened by an observable from the first argument and closed by an observable returned by the function in the second argument', async () => {
    await rxTest(({ hot, observable, expectObservable }) => {
      ...
      expectObservable(
        ...
        [
          { frame: 28, notification: { kind: 'N', value: ['c', 'd', 'e'] } },
          { frame: 28, notification: { kind: 'N', value: [] } },
          { frame: 50, notification: { kind: 'N', value: ['i'] } },
          { frame: 50, notification: { kind: 'C' } },
        ],
        values
      );
    });
