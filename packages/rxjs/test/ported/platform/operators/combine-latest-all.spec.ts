...
describe('combineLatestAll (platform)', () => {
  ...
  it('should work with two nevers', async () => {
    await rxTest(({ observable, expectObservable, expectSubscriptions }) => {
      ...
      const e2subs = '^!';
      const expected = '-';
      const result = Observable.from([e1, e2])[combineLatestAll]((x, y) => x + y);
  ...
  it('should work with hot-single and hot-empty', async () => {
    await rxTest(({ hot, expectObservable, expectSubscriptions }) => {
      ...
      const e2subs = '   ^---!';
      const expected = ' ----|';
      const result = Observable.from([e2, e1])[combineLatestAll]((x, y) => x + y);
      expectObservable(result).toBe(expected);
      expectSubscriptions(e1.subscriptions).toBe(e1subs);
      expectSubscriptions(e2.subscriptions).toBe(e2subs);
    });
  ...
  it('should combine 3 observables', async () => {
    await rxTest(({ hot, expectObservable, expectSubscriptions }) => {
      ...
      const e3subs = '     ^---------!';
      const expected = '   -----wxyz-|';
      const result = Observable.from([e1, e2, e3])[combineLatestAll]((x, y, z) => x + y + z);
  ...
  it('should handle throw after complete left', async () => {
    await rxTest(({ hot, expectObservable, expectSubscriptions }) => {
      ...
      const rightSubs = '     ^--------!';
      const expected = '      ---------#';
      const result = Observable.from([left, right])[combineLatestAll]((x, y) => x + y);
  ...
  it('should handle selector throwing', async () => {
    await rxTest(({ hot, expectObservable, expectSubscriptions }) => {
      ...
      const e2subs = '     ^--!';
      const expected = '   ---#';
      const result = Observable.from([e1, e2])[combineLatestAll]((x, y) => {
        throw 'ha ha ' + x + ', ' + y;
      });
