...
describe('window (platform)', () => {
  ...
  it('should handle basic case with window closings', async () => {
    await rxTest(({ hot, observable, expectObservable, expectSubscriptions }) => {
      ...
      const a = observable('         -3-4|                    ');
      const b = observable('             -5-6|                ');
      const c = observable('                 -7-8|            ');
      const d = observable('                     -9-|         ');
