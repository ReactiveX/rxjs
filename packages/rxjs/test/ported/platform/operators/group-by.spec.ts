...
describe('groupBy (platform)', () => {
  it('should group numbers by odd/even', async () => {
    ...
    await rxTest(({ observable, hot, expectObservable }) => {
      ...
      const expected = '--x---y---------------|';
      const x = observable('  1-------3-------5---|');
      const y = observable('  2-------4-------|');
  ...
  it('should group values with a keySelector', async () => {
    ...
    await rxTest(({ observable, hot, expectObservable, expectSubscriptions }) => {
      ...
      const w = observable('         a-b---d---------i-----l-|', values);
      const x = observable('             c-------g-h---------|', values);
      const y = observable('                 e---------j-k---|', values);
      const z = observable('                   f-------------|', values);
  ...
  it('should not break unsubscription chain when unsubscribed explicitly', async () => {
    ...
    await rxTest(({ hot, expectObservable, expectSubscriptions }) => {
      ...
      const source = e1[mergeMap]((x) => Observable.from([x]))
        ...
        [mergeMap]((group) => Observable.from([group.key]));
      expectObservable(source, unsub).toBe(expected, expectedValues);
      expectSubscriptions(e1.subscriptions).toBe(e1subs);
    });
  });
  it('should group values with a keySelector which eventually throws', async () => {
    ...
    await rxTest(({ observable, hot, expectObservable, expectSubscriptions }) => {
      ...
      const e1subs = '       ^-------------------!';
      const expected = '     --w---x---y-z-------#';
      const w = observable('         a-b---d---------i-#', values);
      const x = observable('             c-------g-h---#', values);
      const y = observable('                 e---------#', values);
      const z = observable('                   f-------#', values);
      const expectedValues = { w: w, x: x, y: y, z: z };
      let invoked = 0;
  ...
  it('should group values with a keySelector and elementSelector, but elementSelector throws', async () => {
    ...
    await rxTest(({ observable, hot, expectObservable, expectSubscriptions }) => {
      ...
      const e1subs = '       ^-------------------!';
      const expected = '     --w---x---y-z-------#';
      const w = observable('         a-b---d---------i-#', reversedValues);
      const x = observable('             c-------g-h---#', reversedValues);
      const y = observable('                 e---------#', reversedValues);
      const z = observable('                   f-------#', reversedValues);
      const expectedValues = { w: w, x: x, y: y, z: z };
      let invoked = 0;
  ...
  it('should allow using a keySelector, elementSelector, and durationSelector', async () => {
    ...
    await rxTest(({ observable, hot, expectObservable, expectSubscriptions }) => {
      ...
      const v = observable('       a-b---(d|)                 ', reversedValues);
      const w = observable('             c-------g-(h|)       ', reversedValues);
      const x = observable('                  e---------j-(k|)', reversedValues);
      const y = observable('                   f-------------|', reversedValues);
      const z = observable('                         i-----l-|', reversedValues);
  ...
  it('should allow using a keySelector and a durationSelector, outer throws', async () => {
    ...
    await rxTest(({ observable, hot, expectObservable, expectSubscriptions }) => {
      ...
      const e1subs = '       ^-------------------------!';
      const expected = '     --v---w---x-y-----z-------#';
      const v = observable('         a-b---(d|)               ', values);
      const w = observable('             c-------g-(h|)       ', values);
      const x = observable('                 e---------j-(k|) ', values);
      const y = observable('                   f-------------#', values);
      const z = observable('                         i-----l-#', values);
      const expectedValues = { v: v, w: w, x: x, y: y, z: z };
      const source = e1[groupBy]((val) => val.toLowerCase().trim(), {
  ...
  it('should dispose a durationSelector after closing the group', async () => {
    ...
    await rxTest(({ observable, hot, expectSubscriptions, time: parseTime, schedule: scheduleAt }) => {
      const obs = hot(' -0-1--------2-| ');
      const sub = '     ^--------------!';
      // prettier-ignore
      const unsubs = [
      ...
      const dur = '     ---s';
      const durations = [observable(dur), observable(dur), observable(dur)];
  ...
  it('should allow using a durationSelector, but keySelector throws', async () => {
    ...
    await rxTest(({ observable, hot, expectObservable, expectSubscriptions }) => {
      ...
      const v = observable('         a-b---(d|)               ', values);
      const w = observable('             c-------g-(h|)       ', values);
      const x = observable('                 e---------#      ', values);
      const y = observable('                   f-------#      ', values);
      const z = observable('                         i-#      ', values);
      const expectedValues = { v: v, w: w, x: x, y: y, z: z };
      let invoked = 0;
      const source = e1[groupBy](
        (val) => {
  ...
  it('should allow using a durationSelector which eventually throws', async () => {
    ...
    await rxTest(({ observable, hot, expectObservable, expectSubscriptions }) => {
      ...
      const v = observable('         a-b---(d|)               ', values);
      const w = observable('             c-----#              ', values);
      const x = observable('                 e-#              ', values);
      const y = observable('                   #              ', values);
