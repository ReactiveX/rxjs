...
describe('sequenceEqual (platform)', () => {
  ...
  it('should return false for two sync observables that are unequal in length', async () => {
    ...
    await rxTest(({ observable, expectObservable }) => {
      const s1 = observable(' (abcdefg|)');
      const s2 = observable(' (abc|)    ');
      const expected = '(F|)      ';
      const source = s1[sequenceEqual](s2);
      expectObservable(source).toBe(expected, booleans);
    });
  ...
  it('should return compare hot and cold observables', async () => {
    ...
    await rxTest(({ hot, observable, expectObservable, expectSubscriptions }) => {
      ...
      const s2 = observable('     ----b---c-|                                 ');
      const s2subs = '      ^---------!                                 ';
      const expected1 = '   ------------(F|)                            ';
      const s3 = observable('                        -f---g---h---i---j---|   ');
      const test2subs = '   -------------------^                        ';
      const expected2 = '   ----------------------------------------(T|)';
      const s3subs = '      -------------------^--------------------!   ';
