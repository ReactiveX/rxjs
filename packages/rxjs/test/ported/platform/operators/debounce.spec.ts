...
describe('debounce (platform)', () => {
  ...
  it('should delay all element by selector observable', async () => {
    await rxTest(({ hot, expectObservable, expectSubscriptions }) => {
      ...
      expectObservable(e1[debounce](() => Observable[timer](2))).toBe(expected);
      expectSubscriptions(e1.subscriptions).toBe(e1subs);
    });
  });
  ...
  it('should support a scalar selector observable', async () => {
    await rxTest(({ hot, expectObservable, expectSubscriptions }) => {
      ...
      expectObservable(e1[debounce](() => Observable.from([0]))).toBe(expected);
      expectSubscriptions(e1.subscriptions).toBe(e1subs);
    });
  });
  ...
  it('should allow unsubscribing early and explicitly', async () => {
    await rxTest(({ hot, expectObservable, expectSubscriptions }) => {
      ...
      const expected = '----a---       ';
      const unsub = '   -------!       ';
      const result = e1[debounce](() => Observable[timer](2));
  ...
  it('should debounce and does not complete when source does not completes', async () => {
    await rxTest(({ hot, expectObservable, expectSubscriptions }) => {
      ...
      const expected = '----a---c--d-';
      expectObservable(
        e1[debounce](() => Observable[timer](2)),
        '^------------!'
      ).toBe(expected);
      expectSubscriptions(e1.subscriptions).toBe(e1subs);
    });
  ...
  it('should debounce all elements while source emits by selector observable', async () => {
    await rxTest(({ hot, expectObservable, expectSubscriptions }) => {
      ...
      const e1subs = '  ^-------------------!';
      const expected = '--------------------(e|)';
      expectObservable(e1[debounce](() => Observable[timer](4))).toBe(expected);
      expectSubscriptions(e1.subscriptions).toBe(e1subs);
    });
  });
  it('should debounce all element while source emits by selector observable until raises error', async () => {
    await rxTest(({ hot, expectObservable, expectSubscriptions }) => {
      ...
      const e1subs = '  ^------------!';
      const expected = '-------------#';
      expectObservable(e1[debounce](() => Observable[timer](5))).toBe(expected);
      expectSubscriptions(e1.subscriptions).toBe(e1subs);
    });
  });
