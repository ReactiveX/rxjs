...
describe('windowTime (platform)', () => {
  ...
  it('should close windows after max count is reached', async () => {
    await rxTest(({ hot, time, observable, expectObservable, expectSubscriptions }) => {
  ...
  it('should close window after max count is reached with windowCreationInterval', async () => {
    await rxTest(({ hot, observable, expectSubscriptions, expectObservable }) => {
      ...
      const expected = '          x---------y---------z-----|';
      const x = observable('            --a--(b|)                  ');
      const y = observable('                      -de-(f|)         ');
      const z = observable('                                -h--i| ');
      const values = { x, y, z };
  ...
  it('should emit windows given windowTimeSpan', async () => {
    await rxTest(({ hot, observable, time, expectSubscriptions, expectObservable }) => {
      const source = hot('--1--2--^--a--b--c--d--e--f--g--h--|');
      const subs = '              ^--------------------------!';
      const timeSpan = time('     ----------|                 ');
      //  10 frames               0---------1---------2------|
  ...
  it('should emit windows given windowTimeSpan and windowCreationInterval', async () => {
    await rxTest(({ hot, time, observable, expectSubscriptions, expectObservable }) => {
      const source = hot('--1--2--^--a--b--c--d--e--f--g--h--|');
      const subs = '              ^--------------------------!';
      //  10 frames               0---------1---------2------|
      const interval = time('     ----------|                 ');
