...
/**
 * Creates the pipeable `refCount` form of the exact-Symbol `[refCount]` capability.
 *
 * The source is supplied when the returned unary function is composed with
 * `rx`, `pipe`, or another function-composition helper. The result uses the same construction, error-forwarding, and AbortSignal cancellation behavior as the Symbol form.
 *
 * @returns A unary function that applies `[refCount]` to its source.
 */
export function pipeableRefCount<T>(): (source: ConnectableObservable<T>) => Observable<T>;
export function pipeableRefCount(...args: any[]): any {
  return (source: Observable<any>) => Reflect.apply(source[refCount] as (...values: any[]) => any, source, args);
}
