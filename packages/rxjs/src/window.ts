...
/**
 * Creates the pipeable `window` form of the exact-Symbol `[window]` capability.
 *
 * The source is supplied when the returned unary function is composed with
 * `rx`, `pipe`, or another function-composition helper. The result uses the same construction, error-forwarding, and AbortSignal cancellation behavior as the Symbol form.
 *
 * @returns A unary function that applies `[window]` to its source.
 */
export function pipeableWindow<T>(boundaries: ObservableInput<any>): (source: Observable<T>) => Observable<Observable<T>>;
export function pipeableWindow(...args: any[]): any {
  return (source: Observable<any>) => Reflect.apply(source[window] as (...values: any[]) => any, source, args);
}
