...
export type ObservableArrayToValueArray<Sources extends readonly ObservableValue<any>[]> = {
  [K in keyof Sources]: Sources[K] extends ObservableValue<infer T> ? T : never;
};

export type ObservableArrayToValueUnion<T extends readonly ObservableValue<any>[]> = T extends ObservableValue<infer R>[] ? R : never;
