...
export class OperatorDecisionTreeService {
  ...
  options$: Observable<OperatorTreeNode[]> = combineLatest(this.tree$, this.state$).pipe(
    ...
    map(([tree, state]) => {
      ...
      return nodeHasOptions(node) ? node.options.map((option) => tree[option]) : tree['initial'].options.map((option) => tree[option]);
    })
  );

  isBeyondInitialQuestion$: Observable<boolean> = this.state$.pipe(map(({ currentBranchId }) => currentBranchId !== 'initial'));
