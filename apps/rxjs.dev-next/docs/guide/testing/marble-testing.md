...
`'-'` or `'------'`: Equivalent to [`NEVER`](/api/functions/NEVER), or an observable that never emits or errors or completes.

`|`: Equivalent to [`EMPTY`](/api/functions/EMPTY), or an observable that never emits and completes immediately.

`#`: Equivalent to [`throwError`](/api/functions/throwError), or an observable that never emits and errors immediately.
