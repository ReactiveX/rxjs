...
functions [`firstValueFrom`](/api/functions/firstValueFrom) or [`lastValueFrom`](/api/functions/lastValueFrom).
...
the return type change), while the `lastValueFrom` will reject with the [`EmptyError`](/api/classes/EmptyError). Thus, the return type of the
...
[`EmptyError`](/api/classes/EmptyError) if the Observable completes with no values emitted.
...
If you don't want Promises created by `lastValueFrom` or `firstValueFrom` to reject with [`EmptyError`](/api/classes/EmptyError) if there
...
function hanging out in memory. To avoid this situation, look into adding something like [`timeout`](/api/operators/timeout), [`take`](/api/operators/take),
[`takeWhile`](/api/operators/takeWhile), or [`takeUntil`](/api/operators/takeUntil) amongst others.
