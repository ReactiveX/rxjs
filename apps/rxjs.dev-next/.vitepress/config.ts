...
export default defineConfig({
  ...
  // Migrated stable guides still use the Angular site's API URL scheme. Keep
  // every non-API link checked while versioned API redirects are designed.
  ignoreDeadLinks: [
    /^\/api(?:\/|$)/,
    /^\.\.?\/api\//,
  ...
  themeConfig: {
    ...
    nav: [
      { text: 'RxJS Next', link: '/next/' },
      { text: 'API Reference', link: '/api' },
      { text: 'RxJS 7 Guides', link: '/guide/overview' },
      {
        text: 'About',
        items: [
          { text: 'Team', link: '/team' },
